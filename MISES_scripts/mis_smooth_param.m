function [p,h,job] = mis_smooth_param(directory,h,col,plot_stuff,is_turb,job)
...
for n = 1:nb
	...
	for n_i = 1:n_inner
		...
		dQ = [0.10; 0.10; 0.10; 0.10; 1.0; 1.0].*dQ;
		job.dQ = dQ;
		...
		if job.knot_coef(7)/job.knot_coef(6)>1.35
			beta = 1.15;
		...
		elseif job.knot_coef(7)/job.knot_coef(6)<1/1.35
			beta = 1.15;
			knot_coef_temp = job.knot_coef(7);
...
% % 		mis_run_inner(directory_smooth)
		...
		if abs(p.Area1)<0.005 && abs(p.Area2)<0.0075 && abs(p.aHb_1)<5*10^-3 && abs(p.aHb_2)<5*10^-3 && abs(p.psi_stag)<2 && abs(p.Alpha-alpha_target)<1.0
			disp('*****Reached smoothness convergence*****')
			break
		end
